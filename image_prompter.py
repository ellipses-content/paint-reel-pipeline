...
# creature ONCE and reuses that exact image in every scene (see image_generator),
# so the monster is identical everywhere — no per-scene extra-limb slop.
CREATURE_SYSTEM = """You design ONE fixed visual look for a cryptid that will be drawn a single time and reused in every scene of a short video.
...
- Give it a clear, simple, drawable anatomy: ONE head, a sensible number of limbs, no extra eyes/horns/tails.
...
def generate_all_prompts(script: list[dict], topic: str) -> list[dict]:
    """Attach the shared creature_design to every block. The creature is drawn
    once and composited into every scene, so there are no per-scene prompts."""

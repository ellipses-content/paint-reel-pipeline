...
# Plain-text endpoint used as a fallback: it returns the answer directly with no
# reasoning/content split, so it can't fail the "empty content" way below.
TEXT_GET_BASE = os.environ.get(
    "POLLINATIONS_TEXT_GET", "https://text.pollinations.ai"
).rstrip("/")
...
def _post(messages: list, max_tokens: int = None, temperature: float = 0.9) -> str:
    ...
    for attempt in range(1, TEXT_MAX_RETRIES + 1):
        ...
        if max_tokens:
            # Escalate the budget each retry: on some topics the reasoning model
            # thinks so long it never emits content (finish_reason=length).
            payload["max_tokens"] = min(int(max_tokens * (1.8 ** (attempt - 1))), 8000)
...
def _get_plain(system: str, user: str) -> str:
    """Fallback: Pollinations' plain-text endpoint. Returns the answer directly
    (no reasoning/content split), so it can't fail the 'empty content' way."""
    prompt = f"{system}\n\n{user}" if system else user
    resp = requests.get(f"{TEXT_GET_BASE}/{quote(prompt)}", timeout=TEXT_TIMEOUT)
    resp.raise_for_status()
    text = (resp.text or "").strip()
    if not text:
        raise ValueError("empty response from plain text endpoint")
    return text
...
def generate_text(system: str, user: str, max_tokens: int = None,
    ...
    Tries the chat endpoint (with escalating token budget); if it still can't
    produce content — some topics make the reasoning model loop forever — falls
    back to the plain-text endpoint so one bad topic can't wedge the pipeline.
    """
    messages = []
    if system:
    ...
    try:
        ...
        print(f"    [text] chat endpoint failed ({e}); trying plain endpoint...")
        return _get_plain(system, user)
